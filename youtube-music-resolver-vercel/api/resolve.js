...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Use youtubei.js's own download() — it uses the library's internal
    // authenticated fetch pipeline so NO raw googlevideo.com URL is ever
    // re-fetched externally. No IP mismatch. No 403.
    const stream = await info.download({
      type: 'audio',
      quality: 'best',
      format: 'mp4',   // AAC/MP4 for Windows Media Foundation compatibility
    });
